...
class ParseHandler(webapp.RequestHandler):
    ...
    def post(self):
        ...
        logging.debug("store key: %s;procssing %s from %s %s"%(
            key,postID,notification[18][0][0][3],memberID))
            ...
            if block[0] not in (4,6):
                ...
                if block[1][0][1]==16:
                    ...
                    if len(notification[18][0][0][7])==2:
                        ...
                        if memberID != u.userID:
                            ...
                            _oid='oid="(\d+)"'
                            logging.debug(repr(notification[18][0][0][7][1][2]))
                            lastMemberID=re.findall(_oid,decode(notification[18][0][0][7][1][2]))
                            if len(lastMemberID)<1:
                                continue
                            lastMemberID=lastMemberID[0]
                            memberID=notification[18][0][0][7][0][6]
                            logging.debug("memberID:%s;userID:%s;lastMemberID:%s"%
                                (memberID,u.userID,lastMemberID))
                    ...
                    if rawString!=None:
                        ...
                        logging.debug("found comment %s"%rawString)
                else:
                    continue
                if rawString==None:
                    continue
                ...
                for single in botMatchList:
                    if single[0]=="default":
                        ...
                        if 1:
                            rst=re.search(single[0],rawString)
                            if rst:
                                print rst
                ...
                if comment:
                    logging.debug("%s#%s"%(postID,comment))
